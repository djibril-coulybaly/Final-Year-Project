...
class TestDatabase(unittest.TestCase):
    def setUp(self):
        self.database = gtfs_data.database.Database(
            GTFS_DATA, INTERESTING_STOPS)
    ...
    def testGetTrip(self):
        ...
        self.assertEqual(found.trip_headsign,
    ...
    def testLoad(self):
        ...
        for unused, t in self.database._trip_db.items():
            ...
            self.assertEqual(t.route['route_short_name'],
    ...
    def testLoadAll(self):
        ...
        database.Load()
        self.assertEqual(database._trip_db.keys(), set(
            ['1167', '1168', '1169', 'ONIGHT']))
    ...
    def testIsValidServiceDay(self):
        ...
        self.assertFalse(database._IsValidServiceDay(
            removed_service_date, '1167'))
        self.assertFalse(database._IsValidServiceDay(
            removed_service_date, '1169'))
        self.assertTrue(database._IsValidServiceDay(
            removed_service_date, '1168'))

        added_service_date = datetime.date(2020, 11, 27)
        self.assertTrue(database._IsValidServiceDay(
            added_service_date, '1167'))
        self.assertTrue(database._IsValidServiceDay(
            added_service_date, '1169'))
        self.assertTrue(database._IsValidServiceDay(
            added_service_date, '1168'))

        normal_service_date = datetime.date(2020, 11, 19)
        self.assertTrue(database._IsValidServiceDay(
            normal_service_date, '1167'))
        self.assertTrue(database._IsValidServiceDay(
            normal_service_date, '1169'))
        self.assertTrue(database._IsValidServiceDay(
            normal_service_date, '1168'))

        normal_no_service_date = datetime.date(2020, 11, 28)
        self.assertFalse(database._IsValidServiceDay(
            normal_no_service_date, '1167'))
        self.assertFalse(database._IsValidServiceDay(
            normal_no_service_date, '1169'))
        self.assertFalse(database._IsValidServiceDay(
            normal_no_service_date, '1168'))
