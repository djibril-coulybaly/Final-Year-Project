/*
  ...
  File Name: search_by_stop_number.dart
  Description: This file contains a searchable ListView widget that will display a list of stops from Dublin Bus, Go Ahead
               Iarnrod Eireann and Luas. The user can search for a route based on its Stop Number. Each stop will contain 
...
class SearchByStopNumber extends StatefulWidget {
  const SearchByStopNumber({Key? key}) : super(key: key);

  @override
  State<SearchByStopNumber> createState() => _SearchByStopNumberState();
}

class _SearchByStopNumberState extends State<SearchByStopNumber> {
  ...
  */
  void searchForRoute(String query) {
    final suggestions = searchResults.where((route) {
      final routeName = route.stopNumber.toLowerCase();
