...
class Database:
    ...
    def _IsValidServiceDay(self, dt: datetime.date, trip_id: str) -> bool:
        ...
        if not service:
            logging.error('service "%s" not found in database',
                          trip.service_id)
            return False
    ...
    def GetScheduledFor(self, stop_id: str, start: datetime.datetime, end: datetime.datetime):
        ...
        possibility = collections.namedtuple(
            'possibility', ['service_date', 'arrival_time'])
        ...
        for s in stops:
            ...
            try:
                ...
                hour, minute, second = [int(x)
                                        for x in arrival_time_str.split(':')]
    ...
    def _LoadTrips(self) -> Dict[str, Trip]:
        ...
        for trip_id, row in trips.items():
            ...
            if route_id not in routes:
                logging.debug(
                    'Trip "%s" references unknown route_id "%s"', trip_id, route_id)
    ...
    def _LoadExceptions(self) -> Dict[str, Dict[datetime.date, str]]:
        ...
        dates = self._Collect(self._Load(
            'calendar_dates.txt'), 'service_id', multi=True)
