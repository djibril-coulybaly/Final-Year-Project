...
def delta_seconds(now: datetime.time, then: datetime.time) -> float:
    ...
    def td(t): return datetime.timedelta(
...
class Upcoming(NamedTuple):
    ...
    def FromTrip(cls, trip: gtfs_data.database.Trip, stop_id: str, source: str, due: str, currentTime: datetime.time):
        return cls(
            ...
            agencyName=trip.route['agency_id'],
            stop_id=stop_id,
            dueTime=due,
...
class Transit:
    ...
    def GetScheduled(self, interesting_stops: List[str]) -> List[Upcoming]:
        ...
        for stop_id in interesting_stops:
            ...
            for t in trips:
                ...
                ret.append(Upcoming.FromTrip(
                    t, stop_id, 'SCHEDULE', due, now().time()))
    ...
    def GetLive(self, interesting_stops: List[str]) -> List[Upcoming]:
        ...
        for e in resp.entity:
                ...
                if int(stu.stop_sequence) <= sequence:
                    if stu.HasField('arrival'):
                        if stu.arrival.HasField('delay'):
                            ...
                            updated_arrival_time += datetime.timedelta(
                                seconds=secs)
            ...
            ret.append(Upcoming.FromTrip(
                trip_from_db, stop_id, 'LIVE', due, current.time()))
